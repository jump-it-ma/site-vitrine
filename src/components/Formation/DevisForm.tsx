...
export default function DevisForm({ formation, formationDate }: Props) {
  ...
  const handleSendDevis = async (args: DevisProps) => {
    const searchFormation = formationsData.find(
      (element) => element.title.toLowerCase() === formation?.toLowerCase()
    );
    if (searchFormation) {
      try {
        setLoading(true);
        ...
        triggerToaster(
          "success",
          "Votre demande de devis a été envoyé avec succés"
        );
        setDevisInputs({
          // Wipe the devis inputs Data to prevent spam
          formation: "",
          entreprise: "",
          fullname: "",
          telephone: "",
  ...
  const handleSubmit = async () => {
    if (
      (formation !== "" || devisInputs.formation !== "") &&
      devisInputs.entreprise !== "" &&
      devisInputs.telephone !== "" &&
      devisInputs.email !== "" &&
      devisInputs.fullname !== ""
    ) {
      if (!loading) {
        ...
        <div className="flex flex-col justify-center items-start w-full">
          ...
          <div className="flex justify-center items-center border border-zinc-300 w-full">
            <input
              ...
              value={
                formation?.length !== 0 ? formation : devisInputs.formation
              }
